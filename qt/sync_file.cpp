...
bool sync(const QString &src, const QString &dst)
{
    ...
    QFileInfoList src_file(src_dir.entryInfoList(QDir::Filter::Files |
                                                 QDir::Filter::Dirs |
                                                 QDir::Filter::NoDotAndDotDot));

    QFileInfoList dst_file(dst_dir.entryInfoList(QDir::Filter::Files |
                                                 QDir::Filter::Dirs |
                                                 QDir::Filter::NoDotAndDotDot));
...
int main(int argc, char **argv)
{
    ...
    parser.addOptions({{"src", "src path", "src"}, {"dst", "dst path", "dst"}});
    parser.process(app);
