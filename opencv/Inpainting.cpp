...
{
    std::cout << "---------------【使用说明】---------------"
                 "\n1、按住鼠标左键对要修复的地方进行选中，被选中的地方将变成白"
                 "色。\n2、按下空格，将对照片进行修复，按下小写s将会对当前图片"
                 "进行保存，文件存储在程序目录下，文件名为Inpainting."
                 "jpg\n3、如不小心选错，可按小写r可恢复原状态\n4、按小写x可以改"
                 "变涂抹线的粗细\n------------------------------------------\n";
    std::cout << "请把要处理的图片放到程序的目录下，然后输入要处理的文件名。\n"
                 "\n也可以直接输入图片路径。\n\n请输入路径或文件名：       "
                 "(注意文件名要由扩展名如：3.jpg或f:\\picture\\3.jpg)\n";
    system("color 1E");
    std::cin >> str;
    original = imread(str);
    while (!original.data) {
    ...
    while (1) {
        ...
        case 's':
            ...
            // MessageBox(NULL, TEXT("图片已保存，请查看"), TEXT("提示"),
            // MB_OK);
            exit(0);
        case 'x':
            lineSize++;
