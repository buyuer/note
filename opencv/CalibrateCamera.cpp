...
void Calibrate_Camera(char num)
{
    cout << "按下空格采集一张照片" << endl
         << "按下v计算相机内存和畸变并显示校正后镜头" << endl
         << "按下esc退出" << endl;
    VideoCapture camera;
    Mat image;
    Mat image1;
    ...
    while (true) {
        camera >> image;
        if (flag == 1) {
            undistort(image, image1, g_CameraMatrix, g_DistCoeffs,
                      g_CameraMatrix);
        ...
        if (key == 'v') {
            ...
            calibrateCamera(object_points, g_Corners, g_ImageSize,
                            g_CameraMatrix, g_DistCoeffs, g_rvecsMat,
                            g_tvecsMat, 0);
...
void Find_Chess_Board_Corners(Mat& image, vector<vector<Point2f>>& corners)
{
    ...
    if (findChessboardCorners(image, g_CornersSize, tempCorners) == 0) {
        ...
        drawChessboardCorners(image, g_CornersSize, g_Corners[g_imageCount++],
                              true);
        imshow("效果图", image);
...
void Get_Camera_Argument()
{
    ...
    calibrateCamera(object_points, g_Corners, g_ImageSize, g_CameraMatrix,
                    g_DistCoeffs, g_rvecsMat, g_tvecsMat, 0);
    cout << "已计算得到的相机参数: " << endl;
    cout << "相机内参：" << endl << g_CameraMatrix << endl;
    cout << "相机畸变：" << endl << g_DistCoeffs << endl;
}